...
Version: 2.0
Author: Instinct Entertainment, Mychelle
...
include('widgets/my-subscriptions.php');

// edit this line to add your bbpress prefix
define("WPSC_BB_PREFIX", "bb_");


add_action('admin_init', 'vl_wpscsm_admin_init');
	
	/**
	 * The init function for this plugin
	 * @access to admins
	 *
	 * @since 0.2
	 */
	function vl_wpscsm_admin_init() {
		/* Do something */
	}
	


	//Set default capabilities if the option is empty
	$wpsc_product_capability_list = get_option('wpsc_product_capability_list');
	if(empty($wpsc_product_capability_list) || (count($wpsc_product_capability_list) < 1)) {
		$wpsc_product_capability_list = array(
			'posts-base' => array(
				'name' => __('Basic Membership Access','wpsc'),
				'capability-type' => "wordpress",
				'subscription-length' => "wordpress"
			),
			'posts-premium' => array(
				'name' => __('Premium Membership Access','wpsc'),
				'capability-type' => "wordpress",
				'subscription-length' => "wordpress"
			),
		/*
	'forums-premium' => array(
				'name' => __('Premium Forums Access','wpsc'),
				'capability-type' => "bbpress"
			)
*/
		);
...
//			echo date('d M Y', $members_lengths['posts-gold']);
//			exit('<pre>'.print_r($members_lengths,true).'</pre>');
			//$subscription_length = $members_lengths - $members_starts;
			//Save users subscription end times
...
add_action('edit_page_form', 'wpsc_post_required_capabilities');
	...
	//exit('<pre>'.print_r($_POST,1).'</pre>');
		...
		if($_POST['page_action'] == 'add' && !empty( $_POST['capability_list']['new']['capability']) && !empty($_POST['capability_list']['new']['name']) ) {
				...
				$new_capability_item[$new_capability]['owner'] = $submitted_capability_data['new']['owner'];
				$new_capability_item[$new_capability]['default'] = $submitted_capability_data['new']['default'];
			  ...
			  foreach($submitted_capability_data as $submitted_capability_key => $submitted_capability_item) {
					...
					$wpsc_product_capability_list[$submitted_capability_key]['owner'] = $submitted_capability_data[$submitted_capability_key]['owner'];
					$wpsc_product_capability_list[$submitted_capability_key]['default'] = $submitted_capability_data[$submitted_capability_key]['default'];
$wpsc_product_capability_list[$submitted_capability_key]['capability-type'] = $submitted_capability_data[$submitted_capability_key]['capability-type'];
			...
			//echo "<pre>".print_r($wpsc_product_capability_list, true)."</pre>";
			update_option('wpsc_product_capability_list', $wpsc_product_capability_list);
	}
